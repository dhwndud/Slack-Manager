...
# Format message from Slack to send to OpenAI [Slack 에서 OpenAI로 보낼 형식 메시지]
def format_openai_message_content(content: str, translate_markdown: bool) -> str:
    ...
    # Convert from Slack mrkdwn to markdown format [Slack mrkdwn에서 마크다운 형식으로 변환]
...
def start_receiving_openai_response(
    *,
    ...
    # max_tokens를 위한 공간이 있는지 확인하기 위해 오래된 메시지를 제거하세요.
    ...
    # TODO: currently we don't pass gpt-4 to this calculation method - 현재 이 계산 방법에 gpt-4를 전달 하지 않음.
...
def calculate_num_tokens(
    messages: List[Dict[str, str]],
    # TODO: adjustment for gpt-4 [gpt-4 수정]
    model: str = GPT_3_5_TURBO_0301_MODEL,
) -> int:
    """Returns the number of tokens used by a list of messages."""
    """메세지 목록에서 사용된 TOKEN 수를 반환 합니댜."""
    try:
        encoding = tiktoken.encoding_for_model(model)
    except KeyError:
        encoding = tiktoken.get_encoding("cl100k_base")
    if model == GPT_3_5_TURBO_0301_MODEL:
        # note: future models may deviate from this [미래 Model은 이것에서 벗어날 수 있다.]
        num_tokens = 0
